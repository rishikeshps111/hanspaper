...
class CompanyServiceProvider extends ServiceProvider
{
    ...
    {
        if (config('app.installation_status')) {
    ...
    public function boot(): void
    {
        if (env('INSTALLATION_STATUS') && Schema::hasTable('company')) {
            ...
            Carbon::setLocale($company['language_code'] ?? 'en');
             ...
             * Email setup
             */
            Config::set('mail.from.address', $company['email'] ?? 'default@example.com');
            Config::set('mail.from.name', $company['name'] ?? 'Default Company');
        }
    }
}
