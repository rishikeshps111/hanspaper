    ...
    public function updateItemMasterAveragePurchasePrice(array $itemIds){
        ...
        foreach ($groupedItemTransactions as $itemId => $transactions) {
            ...
            foreach ($transactions as $transaction) {
                ...
                $purchasePrice = $transaction->unit_price;//8,1
                $quantity = $transaction->quantity;//1,10

                // Handle unit conversion
                if ($itemModel->base_unit_id != $transaction->unit_id && $itemModel->secondary_unit_id == $transaction->unit_id && $itemModel->conversion_rate > 0) {
                    $purchasePrice *= $itemModel->conversion_rate;// 1 * 10 = 10
                    $quantity = $quantity / $itemModel->conversion_rate;//10 / 10 = 1
                }
                $totalWeightedPrice += $purchasePrice * $quantity;
