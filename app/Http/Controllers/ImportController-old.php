    ...
    public function importItems(Request $request)
    {
        ...
        try{
            ...
            foreach ($data as $row) {
                ...
                $mrp            = trim($row[8]);
                $msp            = trim($row[9]);
                $salePrice      = trim($row[10]);
                $discountOnSale = trim($row[11]);
                $discountType   = trim($row[12]);
                $wholesalePrice = trim($row[13]);
                $purchasePrice  = trim($row[14]);
                $taxRate        = trim($row[15]);//required
                $taxName        = trim($row[16]);//required
                $taxType        = trim($row[17]);//required
                $opeingStockQty = trim($row[18]);
                $minimumStockQty= trim($row[19]);
                $itemLocation   = trim($row[20]);
                $baseUnit       = trim($row[21]);//required
                $secondaryUnit  = trim($row[22]);
                $conversionRate = trim($row[23]);
