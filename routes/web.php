...
Route::middleware('auth')->group(function () {
        ...
        Route::group(['prefix' => 'party'], function () {
                ...
                Route::get('/customer/details/{id}', [PartyController::class, 'details'])->name('party.details');
                Route::get('/customer/get-page/{id}', [PartyController::class, 'getPage'])->name('party.details.getPage');
                Route::get('/customer/show/{id}', [PartyController::class, 'show'])->name('party.show');
                Route::get('/customer/production-list/{id}', [PartyController::class, 'productionList'])->name('party.productionList');
                Route::get('/customer/dispatch-list/{id}', [PartyController::class, 'dispatchList'])->name('party.dispatchList');
        });
        ...
        Route::group(['prefix' => 'production'], function () {
                ...
                Route::get('/view/{status}', [ProductionItemMasterController::class, 'FilterList'])->name('production.filterlist');
                  Route::get('/uniquecustomerswithstatus/{status}', [ProductionItemMasterController::class, 'uniqueCustomerswithstatus'])->name('production.unique.customers.withstatus');

                Route::get('/datatable-filter-list', [ProductionItemMasterController::class, 'datatableFilterList'])->name('production.datatable.filter.list');
                Route::post('/assign', [ProductionItemMasterController::class, 'assign'])
                        ->name('item.production.assign');
                Route::get('/print-view/{id}', [ProductionItemMasterController::class, 'printView'])->name('item.production.print.view');
                Route::get('/uniqueproducts', [ProductionItemMasterController::class, 'uniqueProducts'])->name('production.unique.product');
                                Route::get('/uniqueproductswithstatus/{status}', [ProductionItemMasterController::class, 'uniqueProductswithstatus'])->name('production.unique.product.withstatus');
                Route::get('/edit-product/{id}', [ProductionItemMasterController::class, 'editProduct'])->middleware('can:production.edit')
                        ->name('item.editProduct');
                Route::put('/update-product/{id}', [ProductionItemMasterController::class, 'updateProduct'])->middleware('can:production.edit')
                        ->name('item.updateProduct');
                Route::get('/uniquebrands', [ProductionItemMasterController::class, 'uniqueBrands']);
                Route::get('/uniquecategories', [ProductionItemMasterController::class, 'uniqueCategories']);
        ...
        Route::group(['prefix' => 'item'], function () {
                ...
                Route::get('/get-page/{id}', [ItemController::class, 'getPage'])->name('item.detailsone');
                Route::get('/show/{id}', [ItemController::class, 'show'])->name('item.show');
                Route::get('/production-list/{id}', [ItemController::class, 'productionList'])->name('item.productionList');
                Route::get('/dispatch-list/{id}', [ItemController::class, 'dispatchList'])->name('item.dispatchList');
                ...
                Route::get('/ajax/get-multiple', [ItemController::class, 'getMultipleItemsById'])->name('item.ajax.getMultiple');
        ...
        Route::group(['prefix' => 'report'], function () {
                Route::get('/machine', [ReportController::class, 'machineIndex'])
                        ->middleware('can:reports.machine')
                        ->name('machine.report');//View
                Route::post('/machine-report/data', [ReportController::class, 'getMachineData'])
                        ->middleware('can:reports.machine')
                        ->name('machine.report.data');

              Route::get('/produced-by-report', [ReportController::class, 'producedByIndex'])->middleware('can:reports.produced_by')
                        ->name('report.produced_by');
                Route::get('/produced-by-report/production', [ReportController::class, 'producedByIndexProduction'])->middleware('can:reports.produced_by')
                        ->name('report.produced_by.production');
                Route::get('/produced-by-report/production/list', [ReportController::class, 'producedByIndexProductionList'])->middleware('can:reports.produced_by')
                        ->name('report.produced_by.production.list');
                Route::get('/produced-by-report/packing', [ReportController::class, 'producedByIndexPacking'])->middleware('can:reports.produced_by')
                        ->name('report.produced_by.packing');
                Route::get('/produced-by-report/packing/list', [ReportController::class, 'producedByIndexPackingList'])->middleware('can:reports.produced_by')
                        ->name('report.produced_by.packing.list');
                        
                Route::post('/produced-by-report/data', [ReportController::class, 'producedByData'])
                        ->middleware('can:reports.produced_by')
                        ->name('report.produced_by.data');

                Route::get('/real-number-report', [ReportController::class, 'realNumberIndex'])
                        ->middleware('can:reports.real')
                        ->name('report.real_number');
                Route::post('/real-number-report/data', [ReportController::class, 'realNumberData'])
                        ->middleware('can:reports.real')
                        ->name('report.real_number.data');

                Route::get('/packed-by-report', [ReportController::class, 'packedByIndex'])->middleware('can:reports.packed_by')
                        ->name('report.packed_by');
                Route::post('/packed-by-report/data', [ReportController::class, 'packedByData'])
                        ->middleware('can:reports.packed_by')
                        ->name('report.packed_by.data');
        });
        ...
        Route::get('/reals/production-list/{id}', [RealController::class, 'productionList'])->name('reals.productionList');
        Route::get('/reals/stock/{id}', [RealController::class, 'realStock'])->name('reals.stock');
        Route::post('/reals/stock', [RealController::class, 'realStockStore'])->name('reals.stock.store');
        Route::get('reals/modal-create', [RealController::class, 'modalCreate'])->name('reals.modal-create');
        Route::post('reals/modal-store', [RealController::class, 'modalStore'])->name('reals.modal-store');
        Route::resource('reals', RealController::class)->except(['show']);
        Route::get('/reals/report/{id}', [RealController::class, 'report'])->name('reals.report'); 
        Route::get('/reals/finished/', [RealController::class, 'finished'])->name('reals.finished'); 
       Route::get('/reals/alllist/', [RealController::class, 'alllist'])->name('reals.alllist'); 
...
Route::prefix('purchaseorder')->group(function () {
        ...
        Route::post('/cancel', [PurchaseOrderMasterController::class, 'cancel'])->middleware('can:purchaseorder.edit')->name('purchaseorders.cancel');
        ...
        Route::get('/view/{status}', [PurchaseOrderMasterController::class, 'FilterList'])->name('purchaseorder.filterlist');
        Route::get('/uniquecustomerswithstatus/{status}', [PurchaseOrderMasterController::class, 'uniqueCustomerswithstatus'])->name('purchaseorder.unique.customers.withstatus');
        Route::get('/datatable-filter-list', [PurchaseOrderMasterController::class, 'datatableFilterList'])->name('purchaseorder.datatable.filter.list');
        Route::get('/{id}/modifydispatch', [PurchaseOrderMasterController::class, 'modifydispatch'])->name('purchaseorder.modifydispatch');
        Route::post('/save-modifydispatch', [PurchaseOrderMasterController::class, 'savemodifydispatch'])->name('purchaseorder.savemodifydispatch');
         Route::get('/{id}/details', [PurchaseOrderMasterController::class, 'info'])->name('purchaseorder.details');
        Route::get('production-details', [PurchaseOrderMasterController::class, 'infoProduction'])->name('purchaseorder.produaction.details');
...
Route::prefix('dispatch')->group(function () {
        ...
        Route::get('/view/{status}', [DispatchController::class, 'FilterList'])->name('dispatch.filterlist');
         Route::get('/uniquecustomerswithstatus/{status}', [DispatchController::class, 'uniqueCustomerswithstatus'])->name('dispatch.unique.customers.withstatus');
        Route::get('/datatable-filter-list', [DispatchController::class, 'datatableFilterList'])->name('dispatch.datatable.filter.list');
        Route::post('/store-return', [DispatchController::class, 'storeReturn'])->name('dispatch.store.return');
        Route::get('/return-details/{id}', [DispatchController::class, 'returnDetails'])
                ->name('dispatch.return.details');
});
