...
function addRowToInvoiceItemsTable(recordObject, loadedFromUpdateOperation = false) {
    ...
    var productItemId = `<input type="hidden" name="product_item_id[${currentRowId}]" class="form-control" value="${recordObject.id ?? ''}">`;
    var hiddenItemId = `<input type="hidden" name="item_id[${currentRowId}]" class="form-control" value="${recordObject.item_id ?? ''}">`;
    ...
    var inputQuantity = `<input required type="number" name="quantity[${currentRowId ?? ''}]" class="form-control" value="${recordObject.quantity ?? ''}" oninput="checkstatus(this.value, ${recordObject.available_stock ?? ''}, ${currentRowId ?? ''})">`;
    ...
    var remarks = `<textarea rows="5" type="text" name="remarks[${currentRowId ?? ''}]" id="remarks-${currentRowId ?? ''}" class="form-control" placeholder="Remarks">${recordObject.product_remarks ?? ''}</textarea>`;

    var pakingremarks = `<textarea rows="5" type="text" name="pakingremarks[${currentRowId ?? ''}]" id="pakingremarks-${currentRowId ?? ''}" class="form-control" placeholder="Packing Remarks">${recordObject.paking_remarks ?? ''}</textarea>`;

    var dispatchremarks = `<textarea rows="5" type="text" name="dispatchremarks[${currentRowId ?? ''}]" id="dispatchremarks-${currentRowId ?? ''}" class="form-control" placeholder="Dispatch Remarks">${recordObject.paking_remarks ?? ''}</textarea>`;
    ...
    var brand = `<select name="brand_id[${currentRowId}]" class="form-control brand-select" id="brand-select-${currentRowId}" data-selected="${recordObject.item_category_id || ''}">
                    <option value="">Loading brands...</option>
                </select>`;
    var category = `<select name="category_id[${currentRowId}]" class="form-control category-select" id="category-select-${currentRowId}" data-selected="${recordObject.item_category_id || ''}">
                    <option value="">Loading categories...</option>
                </select>`;

    var newRow = $('<tr id="' + currentRowId + '" class="highlight">');
    newRow.append('<td>' + inputDeleteButton + '</td>');
    newRow.append('<td>' + hiddenWarehouseId + hiddenItemId + inputItemName + productItemId + '</td>');
    ...
    if (recordObject.quantity !== undefined && recordObject.quantity !== '') {
        checkstatus(recordObject.quantity, recordObject.available_stock, currentRowId);
    }
    //Serial #2
...
function loadBrandsForRow(rowId) {
    $.ajax({
        ...
        success: function (response) {
            ...
            let selectedBrand = select.data('selected');
            select.empty();
            ...
            $.each(response, function (index, brand) {
                select.append($('<option>', {
                    value: brand.id,
                    text: brand.name,
                    selected: (selectedBrand && selectedBrand == brand.id)
                }));
            });
...
function loadCategoryForRow(rowId) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            let selectedCategory = select.data('selected');
            select.empty();
            ...
            $.each(response, function (index, brand) {
                select.append($('<option>', {
                    value: brand.id,
                    text: brand.name,
                    selected: (selectedCategory && selectedCategory == brand.id)
                }));
            });
...
$(document).ready(function () {
    if (existingItems.length > 0) {
        ...
        $.ajax({
            url: baseURL + '/item/ajax/get-multiple',
            method: 'GET',
            data: {
                product_ids: productIds,
                party_id: $('#party_id').val() // if needed
            },
            success: function (fullItems) {
                ...
                existingItems.forEach(item => {
                    const matched = fullItems.find(i => i.id === item.product_id);
                    if (matched) {
