$(function() {
    ...
    function formAdjustIfSaveOperation(response){
        ...
        var totalWithdrawalAmount = parseFloat(0);
        var totalDepositAmount = parseFloat(0);
        var totalBalance = parseFloat(0);
        
        $.each(response.data, function(index, item) {
            totalWithdrawalAmount += parseFloat(item.withdrawal_amount);
            totalDepositAmount += parseFloat(item.deposit_amount);
            totalBalance = totalDepositAmount - totalWithdrawalAmount;

            tr  +=`
                <tr>
                    <td>${id++}</td>
                    <td>${item.transaction_date}</td>
                    <td>[${item.transaction_details}]${item.party_name}</td>
                    <td class='text-end ' data-tableexport-celltype="number" >${item.withdrawal_amount > 0 ? _formatNumber(item.withdrawal_amount) : ''}</td>
                    <td class='text-end ' data-tableexport-celltype="number" >${item.deposit_amount > 0 ? _formatNumber(item.deposit_amount) : ''}</td>
                    <td class='text-end text-${totalBalance>0 ? "success":"danger"}' data-tableexport-celltype="number" >${_formatNumber(totalBalance)}</td>
                </tr>
            `;
        });
        ...
        tr  +=`
            <tr class='fw-bold'>
                ...
                <td class='text-end' data-tableexport-celltype="number">${_formatNumber(totalWithdrawalAmount)}</td>
                <td class='text-end' data-tableexport-celltype="number">${_formatNumber(totalDepositAmount)}</td>
