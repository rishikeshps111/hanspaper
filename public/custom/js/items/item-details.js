$(function () {
    ...
    function loadDatatables() {
            ...
            columns: [
                { targets: 0, data: 'id', orderable: true, visible: false },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        let brand = full.brand_name || '';
                        let category = full.category_name || '';
                        ...
                        <div class="item-select" data-id="${data}" style="cursor:pointer;">
                            ...
                            ${item_location !== '' ? `
                                <br><span class="badge text-primary bg-light-primary p-2 text-uppercase px-3" 
                            ...
                            <br>
                            <span class="badge bg-light-info text-info mt-1 ms-4">${category}</span>
                            <span class="badge bg-light-success text-success mt-1">${brand}</span>
                        </div>
                        `;
                    }
                }
        ...
        function selectRowById(rowId) {
            ...
            let basePath = currentUrl.split('/item/details/')[0]; // keep base
            let newUrl = basePath + '/item/details/' + rowId;
        ...
        function renderDetailsPanel(rowId) {
            $.ajax({
                url: '/item/show/' + rowId,  // route that returns the blade partial
                type: 'GET',
                beforeSend: function () {
                    $(".details-panel").html(`
    ...
    `);
        ...
        if (selectedId) {
            ...
            $.get(baseURL + '/item/get-page/' + selectedId, function (res) {
