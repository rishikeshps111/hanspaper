...
@section('content')
    ...
    <div class="page-wrapper">
        <div class="page-content">
            <x-breadcrumb :langArray="['Report', 'Real Report']" />
            <div class="row">
                <div class="col-12 col-lg-12">
                    <div class="card">
                        <div class="card-header px-4 py-3">
                            <h5 class="mb-0">{{ __('Real Report') }}</h5>
                        </div>
                        <div class="card-body p-4 row g-3">
                            <div class="col-md-3">
                                <label>Real</label>
                                <select id="real_number" class="form-control single-select-clear-field">
                                    <option value="">Select Real</option>
                                    @foreach ($realNumbers as $r)
                                        <option value="{{ $r->id }}">{{ $r->real_no }}</option>
                                    @endforeach
                                </select>
                            </div>
...
@section('js')
    ...
    <script>
        document.getElementById('generate_report').addEventListener('click', function () {
            const realNumber = document.getElementById('real_number').value;
            const reportType = document.getElementById('report_type').value;
            const fromDate = document.getElementById('from_date').value;
            const toDate = document.getElementById('to_date').value;

            // Simple validation
            if (!realNumber || !reportType || !fromDate || !toDate) {
                iziToast.error({
                    title: 'Error',
                    layout: 2,
            ...
            fetch(`{{ route('report.real_number.data') }}`, {
                method: 'POST',
                headers: {
