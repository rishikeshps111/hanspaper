...
@section('content')
    <div class="page-wrapper">
        <div class="page-content">
            <div class="d-flex justify-content-between align-items-center mb-3">
                <div>
                    <h5 class="mb-0">Core Stock</h5><small class="text-muted">Manage cores and available quantities</small>
                </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle" id="coresTable">
    ...
    <div class="modal fade" id="coreModal" tabindex="-1">
        <div class="modal-dialog">
            <form class="modal-content" id="coreForm">
                @csrf <input type="hidden" id="coreId">
                <div class="modal-header">
                    <h5 class="modal-title">Add Core</h5><button type="button" class="btn-close"
                        data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-6"><label class="form-label">Name <span
                                    class="text-danger">*</span></label><input type="text" maxlength="255"
                                class="form-control" name="name" required>
                            <div class="text-danger small field-error" data-field="name"></div>
                        </div>
                        <div class="col-md-6 opening-field"><label class="form-label">Opening Quantity <span
                                    class="text-danger">*</span></label><input type="number" min="0" class="form-control"
                                name="quantity" value="0">
                            <div class="text-danger small field-error" data-field="quantity"></div>
                        </div>
                ...
                <div class="modal-footer"><button type="button" class="btn btn-light"
                        data-bs-dismiss="modal">Close</button><button class="btn btn-primary" type="submit">Save</button>
                </div>
            </form>
        </div>
    </div>

    <div class="modal fade" id="adjustCoreModal" tabindex="-1">
        <div class="modal-dialog">
            <form class="modal-content" id="adjustCoreForm">
                @csrf <input type="hidden" id="adjustCoreId">
                <div class="modal-header">
                    <h5 class="modal-title">Adjust Core Quantity</h5><button type="button" class="btn-close"
                        data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-6"><label class="form-label">Adjustment</label><select name="adjustment_type"
                                class="form-select" required>
                                <option value="add">Add</option>
                                <option value="remove">Remove</option>
                            </select></div>
                        <div class="col-md-6"><label class="form-label">Quantity</label><input type="number" name="quantity"
                                min="1" class="form-control" required></div>
                        <div class="col-12"><label class="form-label">Remarks</label><textarea name="remarks"
                                class="form-control" required></textarea></div>
                    </div>
                </div>
                <div class="modal-footer"><button type="button" class="btn btn-light"
                        data-bs-dismiss="modal">Close</button><button class="btn btn-primary">Update Quantity</button></div>
            </form>
        </div>
    </div>

    <div class="modal fade" id="coreHistoryModal" tabindex="-1">
        <div class="modal-dialog modal-xl modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Core Stock Transaction History</h5><button type="button" class="btn-close"
                        data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="table-responsive">
                        <table class="table table-bordered align-middle w-100" id="coreHistoryTable">
                            <thead class="table-light">
                                <tr>
                                    <th>Date</th>
                                    <th>Transaction</th>
...
@section('js')
    ...
    <script>
        $(function () {
            ...
            const table = $('#coresTable').DataTable({
                processing: true, serverSide: true, ajax: urls.data, pageLength: 10, lengthMenu: [10, 25, 50, 100], order: [], columns: [
                    { data: 'code', name: 'code' }, { data: 'name', name: 'name' }, { data: 'quantity', name: 'quantity' },
            ...
            $(document).on('click', '.edit-core', function () { const b = $(this); $('#coreForm')[0].reset(); $('#coreId').val(b.data('id')); $('#coreForm [name=name]').val(b.attr('data-name')); $('#coreForm [name=is_active]').val(b.data('active')); $('.opening-field').hide(); $('#coreModal .modal-title').text('Edit ' + b.data('code')); $('#coreModal').modal('show'); });
            ...
            $(document).on('click', '.core-history', function () {
                ...
                historyTable.DataTable({
                    processing: true, serverSide: true, ajax: urls.history.replace('__ID__', button.data('id')), pageLength: 10, lengthMenu: [10, 25, 50, 100], order: [], columns: [
