...
@section('content')
		...
		<div class="page-content">
							...
							<div class="card-body">
								<div class="d-flex align-items-center">
									...
									<div class="widgets-icons-2 rounded-circle bg-gradient-burning text-white ms-auto"><i
											class='bx bxs-down-arrow-circle'></i>
									</div>
								</div>
							</div>
						</div>
							...
							<div class="card-body">
								<div class="d-flex align-items-center">
									...
									<div class="widgets-icons-2 rounded-circle bg-gradient-orange text-white ms-auto"><i
											class='bx bxs-up-arrow-circle'></i>
									</div>
								</div>
							</div>
						</div>
							...
							<ul class="list-group list-group-flush">
								@foreach($trendingItems as $item)
									<li
										...
										{{ $item['name'] }}
										<span
											class="badge bg-success rounded-pill">{{ $formatNumber->formatQuantity($item['total_quantity']) }}</span>
									</li>
								@endforeach
							</ul>
						</div>
								...
								<tbody>
									@foreach($recentInvoices as $recent)
										...
										<tr>
											...
											<td class="text-end">
												{{ $formatNumber->formatWithPrecision($recent->grand_total - $recent->paid_amount) }}
											</td>
			...
			<div class="card mt-4">
				<div class="card-header d-flex align-items-center justify-content-between">
					<h5 class="mb-0">In-Progress Production</h5>
					<div class="d-flex gap-2"><span class="badge bg-success">In Progress:
							{{ $runningMachineCount }}</span><span class="badge bg-danger">Not In Progress:
							{{ $idleMachineCount }}</span></div>
				</div>
				<div class="card-body p-0">
					<div class="table-responsive">
						<table class="table table-bordered table-hover align-middle mb-0">
							<thead class="table-light">
								<tr>
									...
									<th>Reel / Physical Stock Code</th>
									<th width="80" class="text-center">Action</th>
								</tr>
							</thead>
							<tbody>
								@forelse($productionMachines as $machine)
									@php
										$run = $machine->activeProductionRun;
										$production = $run?->production;
										$party = $production?->purchaseOrder?->party;
									@endphp
									<tr class="{{ $run ? 'table-success' : 'table-danger' }}">
										@if($production)
											<td><button type="button"
													class="btn btn-link p-0 fw-semibold view-in-progress-production"
													data-id="{{ $production->id }}">{{ $production->purchaseOrder?->purchase_order_id ?? 'Not Available' }}</button>
											</td>
											<td><button type="button"
													class="btn btn-link p-0 text-start view-in-progress-production"
													data-id="{{ $production->id }}"><span
														class="fw-semibold">#{{ $production->id }}</span><small
														class="d-block text-muted">{{ $production->item?->name ?? 'Not Available' }}</small></button>
											</td>
											<td>{{ trim(($party?->first_name ?? '') . ' ' . ($party?->last_name ?? '')) ?: 'Not Available' }}
											</td>
										@else
											<td>—</td>
											<td>—</td>
											<td>—</td>
										@endif
										<td><strong>{{ $machine->machine_name }}</strong><span
												class="badge d-block mt-1 bg-{{ $run ? 'success' : 'danger' }}">{{ $run ? 'In Progress' : 'Not In Progress' }}</span>
										</td>
										<td>{{ $run?->productionUser?->full_name ?? '—' }}</td>
										<td>
											@if($run)
												<span
													class="badge bg-info text-dark">{{ $run->reelStock?->reel?->code ?? 'Not Available' }}</span>
												<small
													class="d-block text-muted mt-1">{{ $run->reelStock?->stock_code ?? 'Not Available' }}</small>
											@else
												—
											@endif
										</td>
										<td class="text-center">@if($production)<button type="button"
											class="btn btn-sm btn-outline-primary view-in-progress-production"
											data-id="{{ $production->id }}" title="View"><i
										class="bx bx-show"></i></button>@else — @endif
										</td>
									</tr>
								@empty
									<tr>
										<td colspan="7" class="text-center text-muted py-4">No machines found.</td>
									</tr>
								@endforelse
							</tbody>
						</table>
	...
	<div class="modal fade" id="inProgressProductionModal" tabindex="-1" aria-hidden="true">
		<div class="modal-dialog modal-xl modal-dialog-centered">
			<div class="modal-content">
				<div class="modal-header bg-primary text-white">
					<h5 class="modal-title text-white">In-Progress Production Details</h5>
					<button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
						aria-label="Close"></button>
				</div>
				<div class="modal-body" id="inProgressProductionBody"></div>
				<div class="modal-footer">
					<button type="button" class="btn btn-light" data-bs-dismiss="modal">Close</button>
					<a href="#" class="btn btn-primary" id="trackInProgressProduction">Go to Production</a>
				</div>
			</div>
		</div>
	</div>
...
@section('js')
	...
	<script>
		...
		const inProgressDetailsUrl = @json(route('dashboard.in-progress-production.details', ['production' => '__PRODUCTION__'], false));
		const escapeDashboardText = value => $('<div>').text(value ?? 'Not Available').html();
		const formatDashboardQuantity = value => Number(value || 0).toLocaleString(undefined, { maximumFractionDigits: 3 });

		$(document).on('click', '.view-in-progress-production', function () {
			const productionId = $(this).data('id');
			const modal = $('#inProgressProductionModal');
			$('#inProgressProductionBody').html('<div class="text-center py-5"><div class="spinner-border text-primary"></div><div class="mt-2 text-muted">Loading production details...</div></div>');
			$('#trackInProgressProduction').addClass('disabled').attr('href', '#');
			modal.modal('show');

			$.get(inProgressDetailsUrl.replace('__PRODUCTION__', productionId))
				.done(function (data) {
					const detail = (label, value) => '<div class="col-md-3"><small class="text-muted d-block">' + label + '</small><strong>' + escapeDashboardText(value) + '</strong></div>';
					const html = '<div class="row g-3 mb-4">' +
						detail('Work Order ID', data.work_order_id) + detail('Production ID', '#' + data.production_id) +
						detail('Customer', data.customer) + detail('Product', data.product) +
						'</div>' +
						'<div class="card border mb-4"><div class="card-header bg-light fw-semibold">Quantity Progress</div><div class="card-body"><div class="row g-3">' +
						detail('Requested Quantity', formatDashboardQuantity(data.requested_quantity)) +
						detail('Completed Quantity', formatDashboardQuantity(data.completed_quantity)) +
						detail('Current In-Progress Quantity', formatDashboardQuantity(data.current_quantity)) +
						detail('Remaining After Current Run', formatDashboardQuantity(data.remaining_quantity)) +
						'</div></div></div>' +
						'<div class="card border"><div class="card-header bg-light fw-semibold">Current In-Progress Details</div><div class="card-body"><div class="row g-3">' +
						...
						detail('Core Quantity', formatDashboardQuantity(data.core_quantity)) + detail('Output Roll Width', formatDashboardQuantity(data.output_roll_width) + ' ' + data.output_width_unit) +
						detail('Roll Length', formatDashboardQuantity(data.roll_length) + ' ' + data.roll_length_unit) + detail('Started At', data.started_at) +
						'</div></div></div>' +
						...
						(data.completed_runs.length ? data.completed_runs.map(function (run) {
							return '<tr><td><span class="badge bg-success">' + formatDashboardQuantity(run.quantity) + '</span></td><td>' + escapeDashboardText(run.stock_code) + '</td><td>' + escapeDashboardText(run.reel_code) + '</td><td>' + escapeDashboardText(run.machine) + '</td><td>' + escapeDashboardText(run.production_user) + '</td><td>' + escapeDashboardText(run.core_code + (run.core_name ? ' | ' + run.core_name : '')) + '</td><td>' + formatDashboardQuantity(run.core_quantity) + '</td><td>' + (run.output_roll_width === null ? 'Not Available' : formatDashboardQuantity(run.output_roll_width) + ' ' + escapeDashboardText(run.output_width_unit)) + '</td><td>' + (run.roll_length === null ? 'Not Available' : formatDashboardQuantity(run.roll_length) + ' ' + escapeDashboardText(run.roll_length_unit)) + '</td><td>' + escapeDashboardText(run.completed_at) + '</td></tr>';
					...
					$('#trackInProgressProduction').removeClass('disabled').attr('href', data.track_url);
				})
				.fail(function (xhr) {
					$('#inProgressProductionBody').html('<div class="alert alert-danger mb-0">' + escapeDashboardText(xhr.responseJSON?.message || 'Unable to load production details.') + '</div>');
				});
		});
